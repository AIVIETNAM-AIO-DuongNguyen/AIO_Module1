...
images are scarce. Open a page from the sidebar to inspect results, compare lever
combinations, launch single runs, or orchestrate the full experiment matrix.
"""
)

overview_col, progress_col = st.columns([3, 2], gap="large")
...
"""
    ...
    lever_col, matrix_col = st.columns(2, gap="medium")
...
with progress_col:
    with st.container(border=True):
        ...
        if results_table.found:
            ...
            metric_cols[0].metric("Unique runs", f"{completion.unique_runs} / {completion.expected}")
            metric_cols[1].metric("Rows in CSV", completion.completed)
